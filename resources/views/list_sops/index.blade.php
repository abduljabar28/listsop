    ...
    <div class="col-md-12">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
            <div>
                ...
                <h1 class="mb-0">📋 Daftar SOP</h1>
                <p class="text-muted mb-0">Kelola dan lihat detail SOP dengan cepat dari satu tempat.</p>
            </div>
        ...
        <div class="mb-4">
            <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group" role="group" aria-label="Filter Unit Kerja">
                    <a href="{{ route('list_sop.index') }}" class="btn btn-outline-primary{{ request()->routeIs('list_sop.index') ? ' active' : '' }}">Semua Unit</a>
                    @foreach ($workUnits as $unit)
                        <a href="{{ route('list_sop.unit', ['workUnit' => \App\Models\ListSop::workUnitSlug($unit)]) }}" class="btn btn-outline-primary">{{ $unit }}</a>
                    @endforeach
                </div>
        ...
        @else
            <div class="alert alert-info text-center py-5 rounded-4" role="alert">
                <h4 class="alert-heading mb-3">📭 Tidak ada SOP</h4>
                <p class="mb-3">Belum ada SOP yang tersedia. Mulai dengan membuat SOP pertama Anda.</p>
                <a href="{{ route('list_sop.create') }}" class="btn btn-success btn-lg px-4">Buat SOP Pertama</a>
            </div>
        @endif
    </div>
</div>
